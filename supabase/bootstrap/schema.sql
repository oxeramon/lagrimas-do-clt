...
-- Backup integral e redução de grants, aplicados após a versão 019.
...
begin
  ...
  if (t,v,f,g,p) is distinct from (25,6,44,32,25) then
    raise exception 'bootstrap incompleto: % tabelas, % views, % funções, % gatilhos, % policies (esperado 25/6/44/32/25)',
      t, v, f, g, p;
  end if;
  ...
  raise notice 'Banco pronto: 25 tabelas, 6 views, 44 funções, 32 gatilhos, 25 policies, RLS em todas.';
end $$;
