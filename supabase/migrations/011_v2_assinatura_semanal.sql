...
-- não cabem nessa chave. A 008 sabia disso e desviou: materializa_assinaturas
-- filtrava frequencia <> 'semanal'. O resultado é que assinatura semanal podia
-- ser CADASTRADA e nunca virava cobrança nenhuma.
--
-- Agora a identidade é a DATA da ocorrência. Mês é recorte de relatório; a
-- cobrança é um evento com data. Para mensal nada muda: duas ocorrências
-- mensais nunca caem no mesmo dia.

alter table public.transacoes
  add column if not exists ocorrencia_em date;

comment on column public.transacoes.ocorrencia_em is
  'A data da ocorrência de uma assinatura. É a identidade dela: mês não serve, porque semanal tem quatro ou cinco no mesmo mês.';

-- As antigas saem ANTES do backfill.
...
alter table public.transacoes drop constraint if exists ocorrencia_tem_data;
alter table public.transacoes
  add constraint ocorrencia_tem_data
  check ((assinatura_id is null) = (ocorrencia_em is null));

alter table public.transacoes drop constraint if exists ocorrencia_unica_por_data;
alter table public.transacoes
...
-- Quem garante a unicidade agora é a data.
