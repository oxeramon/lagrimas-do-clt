...
--
-- O QUE ESTA MIGRAÇÃO NÃO FAZ, e é a parte mais importante: ela NÃO modela
-- crédito rotativo. Não inventa juros, não projeta encargo. Fatura de 1.000 com
-- 400 pagos fica com 600 em aberto, e ponto. Juros sem modelo de juros vira
-- número errado com cara de certo.
--
-- Também NÃO cria crédito por pagamento excedente: pagar mais do que se deve é
-- recusado, não acomodado.
--
-- O DEFEITO: a 005 criou unique (user_id, tipo, item_id, competencia) em
-- liquidacoes. Para dívida, fixa e receita isso é certo -- é o que impede pagar
-- duas vezes (CASO B da ponte). Para fatura, não: ela recebe N pagamentos, e a
-- unique recusava o segundo. paga_fatura já aceitava p_valor desde a 007 e
-- mesmo assim só dava para pagar integralmente, porque o banco não deixava o resto.
--
-- INVARIANTE QUE CONTINUA VALENDO (docs/CONTRATO_CARTAO.md):
--   compra no cartão = consumo; pagamento da fatura = caixa.
-- Compra de 1.000 paga em 400 + 600 é 1.000 de consumo e 1.000 de caixa.
-- Nunca 2.000. Fatiar o pagamento não muda nenhum dos dois lados.
...
-- quem procurar por que a ponte não deixa pagar duas vezes acha no lugar esperado.
...
-- A soma COM SINAL dos itens realizados. Estorno de compra entra negativo e
-- reduz o devido -- foi a 010 que consertou isso, e a regra tem de ser a mesma
-- aqui e na view, senão o limite de pagamento discorda do número da tela.
...
-- REESCRITA por correção, não por gosto: a versão anterior juntava transacoes e
-- liquidacoes na mesma consulta e agrupava. Com UMA liquidação funciona; com N,
-- cada item da fatura aparece N vezes e o `total` sai MULTIPLICADO. O pagamento
-- parcial transformaria o total num número errado sem ninguém tocar no cálculo.
...
as
select
  f.id as fatura_id, f.user_id, f.cartao_id, f.competencia,
  f.abertura, f.fechamento, f.vencimento, f.obs,
...
left join lateral (
  select sum(l.valor) as pago, count(*) as pagamentos
    from public.liquidacoes l
   where l.user_id = f.user_id and l.tipo = 'fatura' and l.item_id = f.id::text
) p on true;
...
-- desfaz_liquidacao(tipo, item_id, competencia) apaga TUDO daquela competência.
-- Para dívida e fixa é o certo, só existe uma. Para fatura com três pagamentos,
-- apagaria os três, e quem clicou queria desfazer um.
--
-- Esta é a operação PRÓPRIA de desfazer pagamento de fatura. O botão genérico
-- de estorno não serve aqui: estornar a saída criaria uma entrada e deixaria a
-- fatura marcada como paga; o certo é desfazer a operação de origem.
