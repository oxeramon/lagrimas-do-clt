...
| `migrations/` | **O registro do que já rodou**, 001 a 014. Migração aplicada é imutável, comentário incluído. O que cada uma fez está em `../docs/MIGRACOES.md` |
| `testes/` | **Suítes que rodam no banco de verdade**, uma por migração que mudou regra de modelo, mais `isolamento.sql`. Abrem em `begin` e fecham em `rollback`: não gravam nada |
