...
export const marcaPagos   = (mes, ids)  => executa(sb().from("pagamentos").insert(ids.map((id) => ({ mes, item_id: id }))), "marcação");
export const desmarcaPagos = (mes, ids) => executa(sb().from("pagamentos").delete().eq("mes", mes).in("item_id", ids), "marcação");
export const marcaPago    = (mes, id)   => executa(sb().from("pagamentos").insert({ mes, item_id: id }), "marcação");
export const desmarcaPago = (mes, id)   => executa(sb().from("pagamentos").delete().eq("mes", mes).eq("item_id", id), "marcação");
export const limpaPagamentosDe = (itemId) => executa(sb().from("pagamentos").delete().eq("item_id", itemId), "marcação");

/* -------------------------------------------------------------- dividas --*/
export const salvaDivida = (corpo, id) => executa(
  id ? sb().from("dividas").update(corpo).eq("id", id)
     : sb().from("dividas").insert(corpo), "dívida");
export const removeDivida = (id) => executa(sb().from("dividas").delete().eq("id", id), "dívida");
export const importaDividas = (linhas) => executa(sb().from("dividas").insert(linhas), "importação");
export const renomeiaCredorNasDividas = (credorId, nome) =>
  executa(sb().from("dividas").update({ credor: nome }).eq("credor_id", credorId), "dívida");

/* ------------------------------------------------------------- credores --*/
export const salvaCredor = (corpo, id) => executa(
  id ? sb().from("credores").update(corpo).eq("id", id)
     : sb().from("credores").insert(corpo), "credor");
export const removeCredor = (id) => executa(sb().from("credores").delete().eq("id", id), "credor");
export const criaCredorRapido = (nome, ordem) => executa(
  sb().from("credores").insert({ nome, tipo: "Pessoa", ativo: true, ordem })
      .select("id").single(), "credor");

/* ------------------------------------------------------------- receitas --*/
export const salvaReceita = (corpo, id) => executa(
  id ? sb().from("receitas").update(corpo).eq("id", id)
     : sb().from("receitas").insert(corpo), "receita");
export const removeReceita = (id) => executa(sb().from("receitas").delete().eq("id", id), "receita");

/* ---------------------------------------------------------------- fixas --*/
export const salvaFixa = (corpo, id) => executa(
  id ? sb().from("fixas").update(corpo).eq("id", id)
     : sb().from("fixas").insert(corpo), "conta fixa");
export const removeFixa = (id) => executa(sb().from("fixas").delete().eq("id", id), "conta fixa");
...
export const informaValorDoMes = (uid, fixaId, mes, valor) => executa(
  sb().from("fixas_mes").upsert({ user_id: uid, fixa_id: fixaId, mes, valor },
                                { onConflict: "user_id,fixa_id,mes" }), "valor do mês");
export const voltaParaMedia = (fixaId, mes) => executa(
  sb().from("fixas_mes").delete().eq("fixa_id", fixaId).eq("mes", mes), "valor do mês");

/* --------------------------------------------------------------- config --*/
export const salvaRenda = (uid, renda) => executa(
  sb().from("config").upsert({ user_id: uid, renda }, { onConflict: "user_id" }), "renda");
